...
public class RCC_Camera : MonoBehaviour {
    ...
    [Range(0f, 1f)] public float TPSRotationDamping = .75f;                     // Rotation movement damper.
    ...
    [Range(-45f, 45f)] public float TPSPitch = 15f;                                     // Pitch angle.
    ...
    public Vector3 TPSOffset = new Vector3(0f, 0f, .1f);   // TPS position offset.
    ...
    [Range(10f, 90f)] public float TPSMinimumFOV = 50f;         // Minimum field of view related with vehicle speed.
    [Range(10f, 160f)] public float TPSMaximumFOV = 70f;            // Maximum field of view related with vehicle speed.
    ...
    private void TPS() {
        ...
        // Then offset by distance behind the new angle
        position += rotation * (-Vector3.forward * TPSDistance);
        position += Vector3.up * TPSHeight;

        // Look at the target
        transform.rotation = rotation * Quaternion.Euler(TPSPitch, 0f, TPSYaw);
