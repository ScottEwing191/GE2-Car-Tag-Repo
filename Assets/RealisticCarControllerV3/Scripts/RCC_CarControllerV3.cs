...
public class RCC_CarControllerV3 : RCC_Core {
	...
	[Range(.02f, .4f)] public float engineFriction = .15f;       // Engine inertia. Engine reacts faster on lower values.
	...
	#region Steering & Steering Assistance
	public bool useSteeringLimiter = true;                              // Applies counter steering when vehicle is drifting. It helps to keep the control fine of the vehicle.
	...
	public bool useSteeringSensitivity = false;
	[Range(0f, 1f)] public float counterSteeringFactor = 1f;                // Counter steering multiplier.
	[Range(1f, 50f)] public float steeringSensitivityFactor = 15f;            // Steering sensitivity multiplier.
	...
	public float gearShiftDownRPM = 3500f;		//	Shifting up when engine RPM is high enough.
	...
	public enum WheelDamage {Off, Visual, Mechanical }
	public WheelDamage wheelDamage;											//	Use wheel damage.
		...
		if (canControl) {
			...
			if (!externalController) {
                ...
                if (useSteeringSensitivity)
                    steerInput = Mathf.Lerp(steerInput, inputs.steerInput, Time.deltaTime * steeringSensitivityFactor);
                else
	...
	private void LimitSteering() {
		...
		maxSteerInput = Mathf.Clamp((1f - (Mathf.Abs(sidewaysSlip) * counterSteeringFactor)), -1f, 1f);		//	Subtract total average sideways slip from max steer input (1f).
		oldSteeringInput = steerInput;		//	Getting difference of previous steer input.
		...
		if (maxSteerInput > 0f)
			...
			steerInput = Mathf.Clamp(steerInput, sign * maxSteerInput / 2, sign * maxSteerInput / 2);

		steeringDifference = steerInput - oldSteeringInput;     //	Getting difference of previous steer input.

		float accel = steeringDifference;
		steerInput += accel * Time.deltaTime;		//	And applying final damped counter input to steerInput.
		
    }

	private void CounterSteering() {
		...
		float sidewaysSlip = 0f;        //	Total sideways slip of rear wheels.

		sidewaysSlip = RearLeftWheelCollider.wheelSlipAmountSideways + RearRightWheelCollider.wheelSlipAmountSideways;
		sidewaysSlip /= 2f;

		if (Mathf.Abs(sidewaysSlip) > .25f)
			driftingNow = true;
		else
			driftingNow = false;
	...
	void OnCollisionEnter (Collision collision){
		...
		if (((1 << collision.gameObject.layer) & damageFilter) != 0) {
			...
			if (colRelVel.magnitude * cos >= minimumCollisionForce) {
				...
				if (wheelDamage != WheelDamage.Off) {
